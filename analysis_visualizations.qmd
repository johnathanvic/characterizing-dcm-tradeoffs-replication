...
p1 <- ggplot(mod_summary_df, aes(y = rel_cpu, x = mean_avg_likelihood, color = model_category)) +
  # Add the frontier line
  geom_line(data = pareto_frontier,
            aes(y = rel_cpu, x = mean_avg_likelihood,
                linetype = "Pareto Frontier"),
            color = "darkblue",
            linewidth = 1,
            alpha = 0.6,
  ...
  geom_point(data = pareto_frontier,
             aes(y = rel_cpu, x = mean_avg_likelihood, fill = model_category),
             shape = 21,
             size = 5,
             stroke = 1.5,
             color = "black",
  ...
  geom_label_repel(data = mod_summary_df %>% filter(model_label %in% models_to_keep),
                   aes(x = mean_avg_likelihood, y = rel_cpu, label = model_label),
                   size = 4,
                   color = "black",
                   fontface = "bold",
                   fill = "white",
  ...
  # scale_y_log10(
    ...
    sec.axis = sec_axis(
      ~ . * reference_values$mean_cpu,
      name = "Evaluation Time (CPU sec.)",
      labels = function(x) round(x, 2)
    )
  ) +
  scale_x_continuous(limits = c(0.42, 0.485),breaks = seq(0.4, 0.5, 0.02)) +
  labs(
    # title = " Predictive Accuracy vs Computational Cost",
    title = "",
    # title = "Pareto Frontier of Computational Cost vs Predictive Accuracy",
    y = "Computational Cost \n (CPU sec. relative to MNL)",
    x = "Predictive Accuracy \n (Avg. Likelihood)"
  ) +
  theme(
    legend.position = "none",
    # legend.margin = margin(t = 5, r = 5, b = 0, l = 5),  # Reduce bottom margin
    axis.text = element_text(size = 10),  # Controls both x and y axis text
    axis.title = element_text(size = 11), # Controls both x and y axis titles
    axis.line.x.bottom = element_line(arrow = grid::arrow(length = unit(0.3, "cm"), 
                                                     ends = "last"),
                                color = "darkgreen", linewidth = 1),
    axis.line.y.left = element_line(arrow = grid::arrow(length = unit(0.3, "cm"), 
...
ggsave("output/Figures/pareto_accuracy.png", width = 4, height = 4, dpi = 300)
...
# Plot 1: Accuracy vs Time
p1 <- ggplot(mod_summary_df, aes(y = rel_cpu, x = pct_improvement_from_random, color = model_category)) +
  # Add the frontier line
  geom_line(data = pareto_frontier,
            aes(y = rel_cpu, x = pct_improvement_from_random,
                linetype = "Pareto Frontier"),
  ...
  # Outline Pareto frontier points with black border
  geom_point(data = pareto_frontier,
             aes(y = rel_cpu, x = pct_improvement_from_random, fill = model_category),
             shape = 21,
             size = 5,
             stroke = 1.5,
             color = "black",
  ...
  geom_label_repel(data = mod_summary_df %>% filter(model_label %in% models_to_keep),
                   aes(x = pct_improvement_from_random, y = rel_cpu, label = model_label),
                   size = 4,
                   color = "black",
                   fontface = "bold",
                   fill = "white",
...
ggsave("output/Figures/pareto_accuracy_random.png", width = 4, height = 4, dpi = 300)
...
# Identify pareto frontier for psi'
...
# Plot 2: Subs vs Time
p2 <- ggplot(mod_summary_df, aes(y = rel_cpu, x = mean_psi, color = model_category)) +
  # Add the frontier line
  ...
  geom_point(data = pareto_frontier_sub,
             aes(y = rel_cpu, x = mean_psi, fill = model_category),
             shape = 21,
             size = 5,
             stroke = 1.5,
             color = "black",
  ...
  geom_label_repel(data = mod_summary_df %>% filter(model_label %in% models_to_keep),
                   aes(x = mean_psi, y = rel_cpu, label = model_label),
                   size = 4,
                   color = "black",
                   fontface = "bold",
                   fill = "white",
  ...
  scale_x_continuous(
    # limits = c(0.42, 0.485),breaks = seq(0.4, 0.5, 0.02)
    ) +
  labs(
    # title = " Predictive Accuracy vs Computational Cost",
    title = "",
...
ggsave("output/Figures/pareto_subs.png", width = 4, height = 4, dpi = 300)
...
reference_values <- summary_df %>%
  ...
  select(mean_elapsed, se_elapsed, mean_cpu, mean_avg_likelihood, se_avg_likelihood, mean_psi, se_psi)
...
ggplot(oos_df, aes(y = model_label, x = exp(avgLLObs_val),
  ...
  geom_segment(aes(x = 1/3, xend = 1/3, y = -Inf, yend = Inf),
               linetype = "dashed", color = "red", inherit.aes = FALSE) +
  annotate("text", x = 1/3, y = Inf, label = "Random Choice (0.33)", 
           color = "red", hjust = 0.5, vjust = -1, size = 3) +
  scale_y_discrete(limits = rev(levels(oos_df$model_label))) +
  scale_x_continuous(limits = c(1/3, NA), expand = expansion(mult = c(0, 0.05))) +
  theme_minimal(base_size = 10) +
  ...
  theme(axis.text.y = element_text(size = 9, hjust = 0, color = label_colors),
        axis.text.x = element_text(size = 9),
        axis.title.x = element_text(size = 10),
        plot.title = element_text(size = 10, face = "bold", margin = margin(t = 0, b = 20)),
        plot.margin = margin(10, 25, 5, 5),
        legend.position = "none")
...
results %>%
  ...
  annotate(
    ...
    x = 13,
    y = 5,
    label = "Positive (green): column model better",
    size = 3.5,
    fill = "white",
    color = "#339933",
    linewidth = NA,
  ...
  annotate(
    ...
    x = 13,
    y = 3.6,
    label = "Negative (red): row model better",
    size = 3.5,
    fill = "white",
    color = "#ff4d4d",
    linewidth = NA,
...
est_wide_df <- est_wide_df %>%
  ...
  mutate(
    ...
    # Transform alpha using binomial softmax (1 / (1 + exp(-alpha)))
    across(starts_with("alpha_"), ~ {
      if_else(is.na(.) | . == "", NA_character_, {
        stars <- gsub("[^*]", "", .)
        alpha_numeric <- as.numeric(gsub("\\*", "", .))
        alpha_transformed <- 1 / (1 + exp(-alpha_numeric))
        paste0(round(alpha_transformed, 3), stars)
      })
    })
  ) %>%
  ungroup() %>%
...
norm_data_table <- mod_summary_df %>%
  mutate(
  ...
  # select(model_label, mean_AL, mean_psi, mean_cpu, norm_accuracy, norm_psi, norm_cpu)
  select(model_label, mean_AL, mean_psi, mean_cpu, rel_cpu, norm_accuracy, norm_psi, norm_cpu)

print(norm_data_table)

write.csv(norm_data_table, file.path(data_folder, "norm_pareto_table.csv"), row.names = FALSE)
