...
model_order <- c("MNL_none_pref",           "NL_NL_pref",           "CNL_CNL_pref",
                 ...
                 "MXL100_none_wtp",          "MXL100_none_pref",
                 "MXL500_none_wtp",          "MXL500_none_pref",
                 "MXL1k_none_wtp",           "MXL1k_none_pref"
                 )
...
model_labels_underscore <- c(
  ...
  "MXL100_none_pref" = "MXL[Pref*','~100~dr.]",
  "MXL500_none_pref" = "MXL[Pref*','~500~dr.]",
  "MXL1k_none_pref"  = "MXL[Pref*','~1*k~dr.]",
  "MXL100_none_wtp"  = "MXL[WTP*','~100~dr.]",
  "MXL500_none_wtp"  = "MXL[WTP*','~500~dr.]",
  "MXL1k_none_wtp"   = "MXL[WTP*','~1*k~dr.]"
) # add "parse = TRUE" to geom_text_repel()
...
# Plot 1: Out-of-Sample Fit
p1 <- ggplot(oos_df, aes(y = model_label, x = exp(avgLLObs_val),
...
# Plot 2: Psi Prime
p2 <- ggplot(psi_df, aes(y = model_label, x = psi_prime)) +
  stat_summary(
    fun = mean,
  ...
  theme_minimal(base_size = 10) +
  labs(title = "Substitution Invariance \n (95% CI)",
       y = NULL,
...
p3 <- ggplot(plot_data, aes(y = model_label, x = mean_cpu)) +
  geom_errorbar(
  ...
  theme_minimal(base_size = 10) +
  labs(
    title = "Function Evaluation Time \n (95% CI)",
    y = NULL,
...
# 
# # Save
# ggsave(file.path(output_dir, paste0(file_prefix, "combined_comparison.png")), 
...
```{r}
# mod_summary_df_efficiency <- mod_summary_df %>%
#   mutate(
#     # Raw efficiency ratios
#     fit_per_cpu = pct_improvement_from_random / mean_cpu ,
#     psi_per_cpu = mean_psi / mean_cpu ,
#     
#     # Normalize relative to MNL
#     fit_per_cpu_norm = fit_per_cpu / fit_per_cpu[model_label == "MNL"],
#     psi_per_cpu_norm = psi_per_cpu / psi_per_cpu[model_label == "MNL"]
#   )
# 
# mod_summary_df_efficiency %>%
#   select(model_label, fit_per_cpu, psi_per_cpu_norm) %>%
#   arrange(desc(fit_per_cpu))
...
#     psi_per_cpu = mean_psi / mean_cpu,
#     
#     # Normalize where MNL = 0 and MXL (Pref, 1k) = 1
#     fit_per_cpu_norm = (fit_per_cpu - fit_per_cpu[model_label == "MNL"]) / 
#                        (fit_per_cpu[model_label == "MXL1k_none_pref"] - fit_per_cpu[model_label == "MNL"]),
#     psi_per_cpu_norm = (psi_per_cpu - psi_per_cpu[model_label == "MNL"]) / 
#                        (psi_per_cpu[model_label == "MXL1k_none_pref"] - psi_per_cpu[model_label == "MNL"])
#   )
# 
# mod_summary_df_efficiency %>%
#   select(model_label, fit_per_cpu_norm, psi_per_cpu_norm) %>%
#   arrange(desc(fit_per_cpu_norm))
```
...
### Full label accuracy vs cpu

```{r}
# PLOT 1: Predictive Accuracy vs Function Time (Pareto Frontier)
# First identify Pareto optimal points
pareto_points <- mod_summary_df %>%
  arrange(desc(pct_improvement_from_random)) %>%
  mutate(is_pareto = FALSE)
...
# Calculate normalized accuracy based on mean_avg_likelihood (0 = worst, 1 = best)
min_likelihood <- min(mod_summary_df$mean_avg_likelihood)
max_likelihood <- max(mod_summary_df$mean_avg_likelihood)

mod_summary_df <- mod_summary_df %>%
  mutate(norm_accuracy = (mean_avg_likelihood - min_likelihood) / (max_likelihood - min_likelihood))

pareto_frontier <- pareto_frontier %>%
  mutate(norm_accuracy = (mean_avg_likelihood - min_likelihood) / (max_likelihood - min_likelihood))

# Manual label positions for better control
label_positions <- mod_summary_df %>%
  mutate(
    label_x = case_when(
      model_label == "MNL" ~ norm_accuracy + 0,
      model_label == "NL" ~ norm_accuracy + 0.05,
      model_label == "LCL (2 cl.)" ~ norm_accuracy - 0.05,
      model_label == "LCL (4 cl.)" ~ norm_accuracy - 0.05,
      model_label == "LCL (6 cl.)" ~ norm_accuracy + 0.0,
      model_label == "LCL (8 cl.)" ~ norm_accuracy + 0.1,
      model_label == "LC-NL (2 cl.)" ~ norm_accuracy + 0.05,
      model_label == "LC-NL (4 cl.)" ~ norm_accuracy - 0.02,
      model_label == "LC-NL (6 cl.)" ~ norm_accuracy + 0.09,
      model_label == "LC-NL (8 cl.)" ~ norm_accuracy + 0.09,
      model_label == "MXL (WTP, 100 dr.)" ~ norm_accuracy - 0.2,
      model_label == "MXL (Pref, 100 dr.)" ~ norm_accuracy - 0.05,
      model_label == "MXL (WTP, 500 dr.)" ~ norm_accuracy - 0.1,
      model_label == "MXL (Pref, 500 dr.)" ~ norm_accuracy - 0.12,
      model_label == "MXL (WTP, 1k dr.)" ~ norm_accuracy - 0.05,
      model_label == "MXL (Pref, 1k dr.)" ~ norm_accuracy - 0.12,
      TRUE ~ norm_accuracy
    ),
    label_y = case_when(
      model_label == "MNL" ~ rel_cpu - 50,
      model_label == "NL" ~ rel_cpu + 50,
      model_label == "LCL (2 cl.)" ~ rel_cpu - 50,
      model_label == "LCL (4 cl.)" ~ rel_cpu - 50,
      model_label == "LCL (6 cl.)" ~ rel_cpu - 50,
      model_label == "LCL (8 cl.)" ~ rel_cpu + 0,
      model_label == "LC-NL (2 cl.)" ~ rel_cpu + 50,
      model_label == "LC-NL (4 cl.)" ~ rel_cpu + 80,
      model_label == "LC-NL (6 cl.)" ~ rel_cpu + 0,
      model_label == "LC-NL (8 cl.)" ~ rel_cpu + 20,
      model_label == "MXL (WTP, 100 dr.)" ~ rel_cpu + 50,
      model_label == "MXL (Pref, 100 dr.)" ~ rel_cpu + 100,
      model_label == "MXL (WTP, 500 dr.)" ~ rel_cpu + 40,
      model_label == "MXL (Pref, 500 dr.)" ~ rel_cpu + 0,
      model_label == "MXL (WTP, 1k dr.)" ~ rel_cpu + 60,
      model_label == "MXL (Pref, 1k dr.)" ~ rel_cpu - 0,
      TRUE ~ rel_cpu
    )
  )

# Plot 1: Accuracy vs Time
p1 <- ggplot(mod_summary_df, aes(y = rel_cpu, x = norm_accuracy)) +
  # Add the frontier line
  geom_line(data = pareto_frontier, 
            aes(y = rel_cpu, x = norm_accuracy,
                linetype = "Pareto Frontier"),
            color = "darkblue", 
            linewidth = 1,
            alpha = 0.6,
            inherit.aes = FALSE) +
  # Original points
  geom_point(alpha = 0.7, size = 5, color = "steelblue") +
  # Highlight Pareto optimal points
  geom_point(data = pareto_frontier,
             aes(y = rel_cpu, x = norm_accuracy),
             shape = 21,
  ...
  geom_segment(data = label_positions,
               aes(x = norm_accuracy, y = rel_cpu,
                   xend = label_x, yend = label_y),
               color = "gray50",
               size = 0.3) +
  # Add text labels at manual positions with white background
  geom_label(data = label_positions,
             aes(x = label_x, y = label_y, label = model_label),
             size = 2.8,
             color = "black",
             fontface = "bold",
             fill = "white",
  ...
  # scale_y_log10(
    ...
    sec.axis = sec_axis(
      ~ . * reference_values$mean_cpu,
      name = "Evaluation Time (CPU seconds)",
      labels = function(x) round(x, 2)
    )
  ) +
  scale_x_continuous(
    ...
    limits = c(0, 1),
    sec.axis = sec_axis(
      ~ . * (max_likelihood - min_likelihood) + min_likelihood,
      name = "Mean Out-of-Sample Average Likelihood ",
      labels = function(x) round(x, 3)
    )
  ) +
  labs(
    title = " Predictive Accuracy vs Computational Cost",
    y = "Computational Cost",
    x = "Predictive Accuracy (normalized)"
  ) +
  theme(
...
### Full label psi vs cpu
```{r}
# PLOT 2: Substitution Distortion vs Function Time (Pareto Frontier)
# Find Pareto frontier for substitution vs time
# (lower psi is better, lower cpu is better)
pareto_points_sub <- mod_summary_df %>%
  arrange(mean_psi) %>%
  mutate(is_pareto = FALSE)

for(i in 1:nrow(pareto_points_sub)) {
  dominated <- FALSE
  for(j in 1:nrow(pareto_points_sub)) {
    if(i != j && 
       pareto_points_sub$mean_psi[j] <= pareto_points_sub$mean_psi[i] &&
       pareto_points_sub$rel_cpu[j] <= pareto_points_sub$rel_cpu[i] &&
       (pareto_points_sub$mean_psi[j] < pareto_points_sub$mean_psi[i] ||
        pareto_points_sub$rel_cpu[j] < pareto_points_sub$rel_cpu[i])) {
      dominated <- TRUE
      break
  ...
  pareto_points_sub$is_pareto[i] <- !dominated
}

pareto_frontier_sub <- pareto_points_sub %>%
  filter(is_pareto) %>%
  arrange(mean_psi)
...
# PLOT 2: Substitution Invariance vs Function Time (Pareto Frontier)
# Find Pareto frontier for substitution vs time
# (HIGHER psi is better, lower cpu is better)
pareto_points_sub <- mod_summary_df %>%
...
# Calculate normalized psi (0 = worst, 1 = best)
min_psi <- min(mod_summary_df$mean_psi)
max_psi <- max(mod_summary_df$mean_psi)

mod_summary_df <- mod_summary_df %>%
  mutate(norm_psi = (mean_psi - min_psi) / (max_psi - min_psi))

pareto_frontier_sub <- pareto_frontier_sub %>%
  mutate(norm_psi = (mean_psi - min_psi) / (max_psi - min_psi))

# Manual label positions for plot 2
label_positions_p2 <- mod_summary_df %>%
  mutate(
    label_x = case_when(
      model_label == "MNL" ~ norm_psi + 0,
      model_label == "NL" ~ norm_psi - 0.02,
      model_label == "LCL (2 cl.)" ~ norm_psi - 0.0,
      model_label == "LCL (4 cl.)" ~ norm_psi + 0.02,
      model_label == "LCL (6 cl.)" ~ norm_psi - 0.01,
      model_label == "LCL (8 cl.)" ~ norm_psi - 0.02,
      model_label == "LC-NL (2 cl.)" ~ norm_psi + 0.0,
      model_label == "LC-NL (4 cl.)" ~ norm_psi + 0.01,
      model_label == "LC-NL (6 cl.)" ~ norm_psi - 0.0,
      model_label == "LC-NL (8 cl.)" ~ norm_psi + 0.1,
      model_label == "MXL (WTP, 100 dr.)" ~ norm_psi - 0.0,
      model_label == "MXL (Pref, 100 dr.)" ~ norm_psi + 0.1,
      model_label == "MXL (WTP, 500 dr.)" ~ norm_psi - 0.15,
      model_label == "MXL (Pref, 500 dr.)" ~ norm_psi - 0.15,
      model_label == "MXL (WTP, 1k dr.)" ~ norm_psi - 0.15,
      model_label == "MXL (Pref, 1k dr.)" ~ norm_psi - 0.15,
      TRUE ~ norm_psi
    ),
    label_y = case_when(
      model_label == "MNL" ~ rel_cpu - 50,
      model_label == "NL" ~ rel_cpu - 50,
      model_label == "LCL (2 cl.)" ~ rel_cpu - 50,
      model_label == "LCL (4 cl.)" ~ rel_cpu - 75,
      model_label == "LCL (6 cl.)" ~ rel_cpu - 40,
      model_label == "LCL (8 cl.)" ~ rel_cpu - 75,
      model_label == "LC-NL (2 cl.)" ~ rel_cpu + 50,
      model_label == "LC-NL (4 cl.)" ~ rel_cpu - 65,
      model_label == "LC-NL (6 cl.)" ~ rel_cpu - 50,
      model_label == "LC-NL (8 cl.)" ~ rel_cpu - 0,
      model_label == "MXL (WTP, 100 dr.)" ~ rel_cpu + 50,
      model_label == "MXL (Pref, 100 dr.)" ~ rel_cpu - 50,
      model_label == "MXL (WTP, 500 dr.)" ~ rel_cpu + 0,
      model_label == "MXL (Pref, 500 dr.)" ~ rel_cpu + 0,
      model_label == "MXL (WTP, 1k dr.)" ~ rel_cpu + 0,
      model_label == "MXL (Pref, 1k dr.)" ~ rel_cpu - 100,
      TRUE ~ rel_cpu
    )
  )

# Plot 2: Substitution vs Time
p2 <- ggplot(mod_summary_df, aes(y = rel_cpu, x = norm_psi)) +
  # Add the frontier line
  geom_line(data = pareto_frontier_sub,
            aes(y = rel_cpu, x = norm_psi,
                linetype = "Pareto Frontier"),
            color = "darkblue",
            linewidth = 1,
            alpha = 0.6,
  ...
  geom_point(alpha = 0.7, size = 5, color = "steelblue") +
  # Highlight Pareto optimal points
  geom_point(data = pareto_frontier_sub,
             aes(y = rel_cpu, x = norm_psi),
             shape = 21,
             size = 5.5,
             stroke = 1.5,
             ...
             inherit.aes = FALSE) +
  # Add segments connecting points to labels
  geom_segment(data = label_positions_p2,
               aes(x = norm_psi, y = rel_cpu,
                   xend = label_x, yend = label_y),
               color = "gray50",
               size = 0.3) +
  # Add text labels at manual positions with white background
  geom_label(data = label_positions_p2,
             aes(x = label_x, y = label_y, label = model_label),
             size = 2.8,
             color = "black",
             fontface = "bold",
  ...
  scale_x_continuous(
    ...
    breaks = seq(0, 1, 0.2),
    sec.axis = sec_axis(
      ~ . * (max_psi - min_psi) + min_psi,
      name = "Substitution Invariance (ψ')",
      labels = function(x) round(x, 3)
    )
  ) +
  labs(
    title = "Substitution Invariance vs Computational Cost",
    y = "Computational Cost",
    x = "Substitution Invariance (normalized)"
  ) +
  theme(
    legend.position = c(0.01, 0.99),
    legend.justification = c(0, 1),
...
### Minimal label accuracy vs cpu

```{r}
# Filter to keep only selected models
# models_to_keep <- c()
models_to_keep <-c("MNL", "LCL (2 cl.)", "LCL (4 cl.)", "LCL (8 cl.)", "LC-NL (8 cl.)",                    "MXL (Pref, 500 dr.)", "MXL (Pref, 1k dr.)")
#
# mod_summary_df_minimal <- mod_summary_df %>%
#   filter(model_label %in% models_to_keep)
#
# # Also filter label_positions and data to only show selected models
# label_positions <- label_positions %>%
#   filter(model_label %in% models_to_keep)

# Add model category column based on model_label
mod_summary_df <- mod_summary_df %>%
  mutate(model_category = case_when(
    model_label == "MNL" ~ "MNL",
    model_label == "NL" ~ "NL",
    grepl("^LCL \\(", model_label) ~ "LCL",
    grepl("^LC-NL \\(", model_label) ~ "LC-NL",
    grepl("^MXL \\(", model_label) ~ "MXL",
    TRUE ~ "Other"
  ))

# Define color palette for model categories
model_colors <- c("MNL" = "#E41A1C", "NL" = "#377EB8", "LCL" = "#4DAF4A",
                  "LC-NL" = "#984EA3", "MXL" = "#FF7F00")
...
label_positions <- mod_summary_df %>%
  mutate(
    label_x = case_when(
      ...
      # model_label == "LC-NL (4 cl.)" ~ norm_accuracy - 0.02,
      # model_label == "LC-NL (6 cl.)" ~ norm_accuracy + 0.09,
      model_label == "LC-NL (8 cl.)" ~ norm_accuracy - 0.0,
      # model_label == "MXL (WTP, 100 dr.)" ~ norm_accuracy - 0.2,
      model_label == "MXL (Pref, 100 dr.)" ~ norm_accuracy - 0.05,
      # model_label == "MXL (WTP, 500 dr.)" ~ norm_accuracy - 0.1,
      model_label == "MXL (Pref, 500 dr.)" ~ norm_accuracy - 0.12,
    ...
    label_y = case_when(
      ...
      # model_label == "NL" ~ rel_cpu + 50,
      model_label == "LCL (2 cl.)" ~ rel_cpu + 50,
      model_label == "LCL (4 cl.)" ~ rel_cpu + 100,
      model_label == "LCL (6 cl.)" ~ rel_cpu + 50,
      model_label == "LCL (8 cl.)" ~ rel_cpu + 0,
      # model_label == "LC-NL (2 cl.)" ~ rel_cpu + 50,
      # model_label == "LC-NL (4 cl.)" ~ rel_cpu + 80,
      # model_label == "LC-NL (6 cl.)" ~ rel_cpu + 0,
      model_label == "LC-NL (8 cl.)" ~ rel_cpu + 50,
      # model_label == "MXL (WTP, 100 dr.)" ~ rel_cpu + 50,
      model_label == "MXL (Pref, 100 dr.)" ~ rel_cpu + 100,
      # model_label == "MXL (WTP, 500 dr.)" ~ rel_cpu + 40,
      model_label == "MXL (Pref, 500 dr.)" ~ rel_cpu + 0,
...
# Plot 1: Accuracy vs Time
p1 <- ggplot(mod_summary_df, aes(y = rel_cpu, x = norm_accuracy, color = model_category)) +
  # Add the frontier line
  geom_line(data = pareto_frontier,
            aes(y = rel_cpu, x = norm_accuracy,
                linetype = "Pareto Frontier"),
            ...
            alpha = 0.6,
            inherit.aes = FALSE) +
  # Original points (now colored by category)
  geom_point(alpha = 0.7, size = 5) +
  # Outline Pareto frontier points with black border
  geom_point(data = pareto_frontier,
             aes(y = rel_cpu, x = norm_accuracy, fill = model_category),
             shape = 21,
             size = 5,
             stroke = 1.5,
             color = "black",
  ...
  scale_fill_manual(
    values = model_colors,
    guide = "none"  # Don't show fill in legend, only color
  ) +
  # # Add segments connecting points to labels
  # geom_segment(data = label_positions %>% filter(model_label %in% models_to_keep),
  #              aes(x = norm_accuracy, y = rel_cpu,
  #                  xend = label_x, yend = label_y),
  #              color = "gray50",
  #              size = 0.3) +
  # # Add text labels at manual positions with white background
  # geom_label(data = label_positions %>% filter(model_label %in% models_to_keep),
  #            aes(x = label_x, y = label_y, label = model_label),
  #            size = 4,
  #            color = "black",
  #            fontface = "bold",
  ...
  #            label.padding = unit(0.15, "lines"),
  #            label.size = 0) +
    # Automatic labels with repel
  geom_label_repel(data = mod_summary_df %>% filter(model_label %in% models_to_keep),
                   aes(x = norm_accuracy, y = rel_cpu, label = model_label),
                   size = 4,
                   color = "black",
                   fontface = "bold",
                   fill = "white",
  ...
  theme(
    ...
    legend.box.spacing = unit(-0.8, "lines"),  # Negative spacing to bring legends very close
    legend.spacing.y = unit(0.1, "lines"),  # Reduce vertical spacing within legends
    legend.text = element_text(size = 12),  # Controls legend text size
    ...
    legend.margin = margin(t = 0, r = 5, b = 0, l = 5),  # Reduce bottom margin
    axis.text = element_text(size = 12),  # Controls both x and y axis text
    axis.title = element_text(size = 14), # Controls both x and y axis titles
    axis.line.x.bottom = element_line(arrow = grid::arrow(length = unit(0.3, "cm"), 
                                                     ends = "last"),
                                color = "darkgreen", linewidth = 1),
    axis.line.y.left = element_line(arrow = grid::arrow(length = unit(0.3, "cm"), 
...
### Minimal label psi vs cpu

```{r}
# Filter to keep only selected models
models_to_keep <- c("MNL", "LCL (4 cl.)", "LCL (8 cl.)", "LC-NL (8 cl.)",
                    "MXL (Pref, 500 dr.)", "MXL (Pref, 1k dr.)")

# Add model category column based on model_label (if not already added)
if (!"model_category" %in% names(mod_summary_df)) {
  mod_summary_df <- mod_summary_df %>%
    mutate(model_category = case_when(
      model_label == "MNL" ~ "MNL",
      model_label == "NL" ~ "NL",
...
# Plot 2: Substitution vs Time
p2 <- ggplot(mod_summary_df, aes(y = rel_cpu, x = norm_psi, color = model_category)) +
  # Add the frontier line
  geom_line(data = pareto_frontier_sub,
            aes(y = rel_cpu, x = norm_psi,
                linetype = "Pareto Frontier"),
  ...
  # Outline Pareto frontier points with black border
  geom_point(data = pareto_frontier_sub,
             aes(y = rel_cpu, x = norm_psi, fill = model_category),
             shape = 21,
             size = 5,
             stroke = 1.5,
             color = "black",
  ...
  # Automatic labels with repel
  geom_label_repel(data = mod_summary_df %>% filter(model_label %in% models_to_keep),
                   aes(x = norm_psi, y = rel_cpu, label = model_label),
                   size = 4,
                   color = "black",
                   fontface = "bold",
                   fill = "white",
...
```{r}
# Create table with primary (normalized) and secondary (actual) axis values
plot_data_table <- mod_summary_df %>%
  mutate(
    mean_cpu = rel_cpu * reference_values$mean_cpu,
    norm_cpu = (rel_cpu - min(rel_cpu)) / (max(rel_cpu) - min(rel_cpu))
  ) %>%
  rename(
    mean_AL = mean_avg_likelihood
  ) %>%
  select(model_label, mean_AL, mean_psi, mean_cpu, norm_accuracy, norm_psi, rel_cpu, norm_cpu)

print(plot_data_table)
```


### Efficiency per cpu sec
```{r}
# Create summary dataframe with key metrics
model_metrics_df <- mod_summary_df %>%
  # select(model_label, norm_accuracy, mean_avg_likelihood, norm_psi, mean_psi, rel_cpu) %>%
  select(model_label, model_category, norm_accuracy, norm_psi, rel_cpu) %>%
  mutate(
    norm_cpu = (rel_cpu - min(rel_cpu)) / (max(rel_cpu) - min(rel_cpu)),
    # accuracy_per_cpu = norm_accuracy / rel_cpu,
    # psi_per_cpu = norm_psi / rel_cpu
    accuracy_per_cpu = norm_accuracy / norm_cpu,
    psi_per_cpu = norm_psi / norm_cpu
    ) %>%
  arrange(desc(accuracy_per_cpu))
...
model_metrics_plot_df <- model_metrics_df %>%  filter(model_label != "MNL")
p1 <- ggplot(model_metrics_plot_df,
             aes(x = reorder(model_label, accuracy_per_cpu),
                 y = accuracy_per_cpu,
                 fill = model_category)) +
  geom_col() +
  geom_text(aes(label = round(accuracy_per_cpu, 1)),
          hjust = -0.1,
          size = 3) +
  coord_flip(clip = "off") +
  scale_fill_manual(
  ...
  labs(
    title = "Efficiency for \nPredictive Accuracy",
    x = "Model",
    y = "Ratio of Normalized \n Accuracy per CPU"
  ) +
  theme_minimal() +
  theme(
...
p2 <- ggplot(model_metrics_plot_df,
             aes(x = reorder(model_label, psi_per_cpu),
                 y = psi_per_cpu,
                 fill = model_category)) +
  geom_col() +
  geom_text(aes(label = round(accuracy_per_cpu, 1)),
...
p1 <- ggplot(mod_summary_df, aes(y = rel_cpu, x = norm_accuracy, color = model_category)) +
  ...
  labs(
    # title = " Predictive Accuracy vs Computational Cost",
    title = "Pareto Frontier of Computational Cost vs Predictive Accuracy",
    y = "Computational Cost",
    x = "Predictive Accuracy (normalized)"
  ) +
  theme(
...
# Filter to keep only selected models
models_to_keep <- c()
# models_to_keep <- c("MNL", "LCL (4 cl.)", "LCL (8 cl.)", "LC-NL (8 cl.)",                    "MXL (Pref, 500 dr.)", "MXL (Pref, 1k dr.)")
...
p2 <- ggplot(mod_summary_df, aes(y = rel_cpu, x = norm_psi, color = model_category)) +
  ...
  labs(
    title = "Pareto Frontier of Computational Cost vs Substitution Invariance",
    # title = "Substitution Invariance vs Computational Cost",
    y = "Computational Cost",
    x = "Substitution Invariance (normalized)"
  ) +
...
```{r}
# Get unique model names and filter out CNL and LC-CNL models
models <- model_labels[model_order]
models <- models[models %in% unique(oos_df$model_label)]
models <- models[!grepl("^CNL|^LC-CNL", models)]  # Filter out CNL and LC-CNL
...
# Create color vectors for axis labels based on model category
get_model_category <- function(label) {
  case_when(
    label == "MNL" ~ "MNL",
    label == "NL" ~ "NL",
    grepl("^LCL \\(", label) ~ "LCL",
    grepl("^LC-NL \\(", label) ~ "LC-NL",
    grepl("^MXL \\(", label) ~ "MXL",
    TRUE ~ "Other"
  )
}

x_colors <- sapply(x_labels, function(l) model_colors[get_model_category(l)])
y_colors <- sapply(y_labels, function(l) model_colors[get_model_category(l)])
...
results %>%
  ...
  labs(
    ...
    subtitle = "Mean percent difference in average likelihood across five holdout samples",
    # subtitle = "Mean % difference in average likelihood \n (* denotes models with a numerical issue for one sample)",
  ...
  theme(
    axis.text.x = element_text(angle = 0, hjust = 0.5, color = x_colors, size = 7, face="bold"),
    axis.text.y = element_text(color = y_colors, size = 7, hjust = 0, face="bold"),
    panel.grid = element_blank(),
...
```{r}
# Create summary dataframe with raw metrics per CPU second
model_metrics_raw_df <- mod_summary_df %>%
  select(model_label, mean_avg_likelihood, mean_psi, mean_cpu) %>%
  mutate(
    likelihood_per_cpu_sec = mean_avg_likelihood / mean_cpu,
    psi_per_cpu_sec = mean_psi / mean_cpu
  ) %>%
  arrange(desc(likelihood_per_cpu_sec))

print(model_metrics_raw_df)

library(ggplot2)
library(patchwork)

p1_raw <- ggplot(model_metrics_raw_df,
                 aes(x = reorder(model_label, likelihood_per_cpu_sec),
                     y = likelihood_per_cpu_sec)) +
  geom_col() +
  coord_flip() +
  labs(
    title = "Avg. Likelihood per CPU Second",
    x = "Model",
    y = "AL / CPU sec"
  ) +
  theme_minimal()

p2_raw <- ggplot(model_metrics_raw_df,
                 aes(x = reorder(model_label, psi_per_cpu_sec),
                     y = psi_per_cpu_sec)) +
  geom_col() +
  coord_flip() +
  labs(
    title = "Psi-Prime per CPU Second",
    x = "Model",
    y = "ψ' / CPU sec"
  ) +
  theme_minimal()
...
# Plot 1: Accuracy vs Time
p1 <- ggplot(mod_summary_df, aes(y = rel_cpu, x = pct_improvement_from_random)) +
  # Add the frontier line
  geom_line(data = pareto_frontier, 
            aes(y = rel_cpu, x = pct_improvement_from_random,
                linetype = "Pareto Frontier"),
  ...
  labs(
    title = "Pareto Frontier: Predictive Accuracy vs Function Time",
    y = "Function Evaluation Time (relative to MNL)",
    x = "Predictive Accuracy (Relative Average Likelihood)"
  ) +
  theme(
...
# Plot 2: Substitution vs Time
p2 <- ggplot(mod_summary_df, aes(y = rel_cpu, x = mean_psi)) +
  # Add the frontier line
  geom_line(data = pareto_frontier_sub, 
            aes(y = rel_cpu, x = mean_psi,
                linetype = "Pareto Frontier"),
            color = "darkred", 
            linewidth = 1,
            alpha = 0.6,
            inherit.aes = FALSE) +
  # Original points (no color mapping)
  geom_point(alpha = 0.7, size = 5, color = "steelblue") +
  # Highlight Pareto optimal points
  geom_point(data = pareto_frontier_sub,
             aes(y = rel_cpu, x = mean_psi),
  ...
  scale_linetype_manual(
    ...
    values = c("Pareto Frontier" = "solid"),
    guide = guide_legend(
      override.aes = list(color = "darkred", linewidth = 1)
    )
  ) +
  theme_minimal() +
  scale_y_log10(
  ...
  labs(
    title = "Pareto Frontier: Substitution Invariance vs Function Time",
    y = "Function Evaluation Time (relative to MNL)",
    x = "Substitution Invariance (ψ')"
  ) +
  theme(
    legend.position = c(0.99, 0.01),
    legend.justification = c(1, 0),
