#!/bin/bash
#SBATCH --job-name=run_big-ms
#SBATCH --array=1-4 # TAG
#SBATCH --partition=RM-shared 
#SBATCH --ntasks-per-node=1      # Prevent full node billing
#SBATCH --cpus-per-task=32     
#SBATCH --time=72:00:00
#SBATCH --output=logs/apollo_ms_%A_%a.out
#SBATCH --error=logs/apollo_ms_%A_%a.err
#SBATCH --mail-type=END,FAIL

# Specify
RUN_SETTINGS="inputs/hpc_main/hpc_batch_settings_main2.R"
MODEL_BATCH_LIST="inputs/hpc_main/hpc_models_list_batch_main2b-big.R" # ENSURE LIST LENGTH MATCHES ARRAY
...
Rscript hpc/hpc_run_ms.R $SLURM_ARRAY_TASK_ID $SLURM_CPUS_PER_TASK $mem_per_core
...
mv logs/apollo_ms_${SLURM_ARRAY_JOB_ID}_${SLURM_ARRAY_TASK_ID}.out logs/job_${SLURM_ARRAY_JOB_ID}/ 2>/dev/null
mv logs/apollo_ms_${SLURM_ARRAY_JOB_ID}_${SLURM_ARRAY_TASK_ID}.err logs/job_${SLURM_ARRAY_JOB_ID}/ 2>/dev/null
