...
custom_apollo_outOfSample <- function(apollo_beta, apollo_fixed,
  ...
  lInSampleStack    <- matrix(0, nrow=nRep, ncol=length(llComponents), 
                               dimnames=list(c(), paste0("inSampleL_", cNam)))
  lOutOfSampleStack <- matrix(0, nrow=nRep, ncol=length(llComponents), 
                               dimnames=list(c(), paste0("outOfSampleL_", cNam)))
  timingEstStack     <- matrix(0, nrow=nRep, ncol=length(llComponents), 
                               dimnames=list(c(), paste0("timingEst_", cNam)))    # Added
  timingOutOfSampleStack <- matrix(0, nrow=nRep, ncol=length(llComponents), 
                                   dimnames=list(c(), paste0("timingOutOfSample_", cNam)))    # Added
  timingCPUTotalIterStack <- matrix(0, nrow=nRep, ncol=length(llComponents), 
                                 dimnames=list(c(), paste0("timingCPUTotalIterStack_", cNam)))    # Added
  timingWallTotalIterStack <- matrix(0, nrow=nRep, ncol=length(llComponents), 
                                    dimnames=list(c(), paste0("timingWallTotalIterStack_", cNam)))    # Added
  ...
  if(file.exists(fileNameParams) && file.exists(fileNameSample)){
    ...
    # tmb3b <- tmp[, grep('^timingOutOfSample_', colnames(tmp)), drop=FALSE] # timingOutOfSampleStack    # Added
    ...
    # test <- test && ncol(tmp3)==ncol(timingOutOfSampleStack)    # Added
    ...
    if(test){
      ...
      # timingOutOfSampleStack <- rbind(tmp3b, timingOutOfSampleStack)    # Added
  ...
  original_model_name <- apollo_control$modelName  # MODIFIED: Store original model name and create CV-specific name
  
  # Loop over each cycle
  for(i in (nRun + 1):nRep){
    ### MODIFIED ###
    # Store original model name and create CV-specific name
    cv_model_name <- paste0(original_model_name, "_CV", sprintf("%02d", i))
    apollo_control$modelName <- cv_model_name
    ### END MODIFIED ###
    
    iter_start_wall_time <- Sys.time()
    iter_start_cpu_time <- proc.time()
    ...
    if(successfulEstimation){
      ...
      ### MODIFIED ###
      # Save model
      apollo_print(paste0("Saving CV model as: ", cv_model_name))
      apollo_saveOutput(model, saveOutput_settings = list(printPVal = FALSE))
      ### END MODIFIED ###
      ...
      ### Start Modified/added ###
      for(j in 1:ncol(llInSampleStack)) llInSampleStack[i,j] <- ifelse(workInLogs, 
                                                                       sum(llin[[j]]), 
                                                                       sum(log(llin[[j]])))
      # for(j in 1:ncol(lInSampleStack)) lInSampleStack[i,j] <- ifelse(workInLogs, 
      #                                                                  exp(sum(llin[[j]])), 
      #                                                                  prod(llin[[j]]))
      # Save llin to CSV
      write.csv(as.data.frame(llin), 
                paste0(apollo_control$outputDirectory, cv_model_name, "_LLin.csv"), 
                row.names = FALSE)
      
      # for(j in 1:ncol(llInSampleStack)) llInSampleStack[i,j] <- ifelse(workInLogs, 
      #                                                                  sum(pmax(llin[[j]], log(1e-300))), 
      #                                                                  sum(log(pmax(llin[[j]], 1e-300)))) # MODIFIED: Add numerical Pr_LB
      # for(j in 1:ncol(llInSampleStack)) llInSampleStack[i,j] <- ifelse(workInLogs,
      #                                                                  sum(llin[[j]]),
      #                                                                  sum(log(llin[[j]])))
      ### END Modified/added ###
      ...
      start_cpu    <- proc.time()
      database2 <- database[samples[,i]>0,]
      og_apollo_control <- apollo_control # ADDED: Stash
      apollo_control$workInLogs = FALSE       # ADDED
      apollo_inputs <- apollo_validateInputs(
        apollo_beta      = apollo_beta, 
        apollo_fixed     = apollo_fixed, 
        database         = database2  , 
        ...
        recycle          = TRUE)
      apollo_inputs$apollo_control$noDiagnostics <- TRUE
      lout <- apollo_probabilities(model$estimate, apollo_inputs, 
                                    functionality="output")
      # Convert llout to match original workInLogs setting
      if(workInLogs){
        llout <- lapply(lout, log)
      } else {
        llout <- lout
      }
      apollo_control <- og_apollo_control # ADDED: Restore
      
      end_cpu   <- proc.time()
      # cpu_time <- timing["user.self"] + timing["sys.self"]
      # cat("Out-of-sample time:", round(difftime(time2, time1, units = "secs"), 3), "seconds\n")
      timingOutOfSample <- (end_cpu - start_cpu)["user.self"] + (end_cpu - start_cpu)["sys.self"]    # CPU time = user + system time in R # Added
      apollo_print(paste0("Out-of-sample time: ", format(timingOutOfSample)))     # Added


      # Restored Original
      for(j in 1:ncol(llOutOfSampleStack)){
        llOutOfSampleStack[i,j] <- ifelse(workInLogs,
                                          sum(llout[[j]]),
                                          sum(log(llout[[j]])))
        # lOutOfSampleStack[i,j] <- ifelse(workInLogs,
        #                                    exp(sum(llout[[j]])), 
        #                                    prod(llout[[j]]))
        timingOutOfSampleStack[i,j] <- timingOutOfSample   # Added
        timingEstStack[i,j] <- model$timeEst # Added
      }
      
      # Save llout to CSV
      write.csv(as.data.frame(llout), 
                paste0(apollo_control$outputDirectory, cv_model_name, "_LLout.csv"), 
                row.names = FALSE)
      ### End Modified/added ###
      ...
      iter_end_wall_time <- Sys.time()
      iter_end_cpu_time <- proc.time()
      timingCPUTotalIter <- (iter_end_cpu_time - iter_start_cpu_time)["user.self"] + (iter_end_cpu_time - iter_start_cpu_time)["sys.self"]    # CPU time = user + system time in R # Added
      timingWallTotalIter <- difftime(iter_end_wall_time, iter_start_wall_time, units='s') # iter_end_wall_time - iter_start_wall_time
      
      apollo_print(paste0("Iter. total wall time: ", format(timingWallTotalIter)))     # Added
      
      for(j in 1:ncol(llOutOfSampleStack)){
        timingCPUTotalIterStack[i,j] <- timingCPUTotalIter
        timingWallTotalIterStack[i,j] <- timingWallTotalIter
      } 
      ...
      percentDiffLL  = 100*(avgLLObs_val-avgObsLL_est)/avgObsLL_est        
      avgObsL_est = exp(avgObsLL_est)
      avgLObs_val = exp(avgLLObs_val)
      # avgObsL_est = lInSampleStack[,"inSampleL_model"]/nObsStack
      # avgLObs_val = lOutOfSampleStack[,"outOfSampleL_model"]/(nrow(database)-nObsStack)
      percentDiffL= -100*(avgLObs_val-avgObsL_est)/avgObsL_est # NEGATED FOR CONSISTENCY W/ AvgLL
      # percentDiffLL  = 100*(avgLLObs_val-avgObsLL_est)/avgObsLL_est
      ...
      utils::write.csv(cbind(paramStack, 
                             ...
                             avgLObs_val,              # Added
                             percentDiffL,             # Added
                             timingEstStack,           # Added
                             timingOutOfSampleStack,   # Added
                             timingWallTotalIterStack,
                             timingCPUTotalIterStack,     # Added
                             inSampleObs=nObsStack,    # Added
  ...
  percentDiffL= -100*(avgLObs_val-avgObsL_est)/avgObsL_est # NEGATED FOR CONSISTENCY W/ AvgLL
  #
  cpuSec_val = timingOutOfSampleStack[,"timingOutOfSample_model"]
  ...
  colnames(M) <- c("LL per obs in estimation sample", 
                   ...
                   "% difference",
                   "CPU sec in validation sample" # Added
                   )
  ...
  # Add AL to matrix
  N <- cbind(avgObsL_est, avgLObs_val, percentDiffL)
  rownames(N)=paste0("sample_",1:nrow(N))
  N <- rbind(N, Average = colMeans(N))
  M <- cbind(M,N)
